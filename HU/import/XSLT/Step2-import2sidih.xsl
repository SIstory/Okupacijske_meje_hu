...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- izhodiščni je list-exhebition\d{2}.xml -->
    <xsl:output method="xml" indent="yes"/>
    
    <xsl:param name="lastSIDIHfileNumber">2489</xsl:param>
    
    <xsl:param name="pathStart">../import/exh</xsl:param>
    ...
    <xsl:template match="root">
        ...
        <xsl:for-each select="figure">
            ...
            <xsl:variable name="document" select="concat($pathStart,$exhNum,$path2entity,$entity-num,'/file',$fileNumber,'/mets.xml')"/>
                    ...
                    <METS:dmdSec ID="description">
                        <METS:mdWrap MDTYPE="DC">
                            <METS:xmlData xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/">
                                ...
                                <xsl:for-each select="creator[string-length(.) gt 0]">
                                    <dc:creator>
                                        <xsl:value-of select="normalize-space(.)"/>
                                    </dc:creator>
                                </xsl:for-each>
                                <xsl:for-each select="description[string-length(.) gt 0]">
                                    <dc:description xml:lang="{@xml:lang}">
                                        <xsl:value-of select="normalize-space(.)"/>
                                    </dc:description>
                                </xsl:for-each>
                                <xsl:for-each select="source[string-length(.) gt 0]">
                                    <dc:source>
                                        <xsl:value-of select="normalize-space(.)"/>
                                    </dc:source>
                                </xsl:for-each>
                                <xsl:for-each select="provenance[string-length(.) gt 0]">
                                    <dcterms:provenance>
                                        <xsl:value-of select="normalize-space(.)"/>
                                    </dcterms:provenance>
                                </xsl:for-each>
